...
The `blendMode` parameter controls how each lyrics line composites against its background layer. The default `BlendMode.Plus` adds the text color to the underlying pixels, which creates a glowing, luminous effect when the background is dark.

```kotlin
KaraokeLyricsView(
    // ...
    blendMode = BlendMode.Plus  // default — additive blending, creates a glow on dark backgrounds
)
```
