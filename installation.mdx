---
title: "Installing Accompanist Lyrics UI in Your Project"
...
<Warning>
  The Android target requires **minSdk 29** because the blur effect applied to inactive lines relies on `RenderEffect` APIs introduced in Android 12 (API 31) with a graceful fallback path that still requires API 29 baseline rendering behaviour. Setting `minSdk` below 29 will cause a build-time manifest merge failure.
</Warning>
