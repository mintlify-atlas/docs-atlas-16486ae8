...
The Accompanist sample app is a complete music player that showcases every major capability of the `lyrics-ui` library. It pairs a `KaraokeLyricsView` with real-time ExoPlayer playback, an animated album-art background, a social sharing flow, and a fully adaptive layout that works on phones, tablets, and the desktop JVM target — giving you a battle-tested reference for your own integration.
...
<Steps>
  ...
  <Step title="Run the desktop JVM target">
    The sample also ships a desktop entry point (`MainKt`). Run it with:

    ```bash
    ./gradlew :sample:run
    ```

    Audio playback on desktop uses the same Media3 abstraction; lyrics rendering is identical to Android.
  </Step>
</Steps>
