...
<Note>
The accompaniment-to-main anchor mapping is computed automatically by `KaraokeLyricsView` based on timestamp proximity. For each `AccompanimentKaraokeLine` in the flat `lyrics.lines` list, the library finds the nearest main line by comparing `line.start` to all non-accompaniment line start times — choosing the closest one whether it comes before or after. You do not need to supply explicit anchor indices in your data model.
</Note>
